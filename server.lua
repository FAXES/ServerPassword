...
-- Shown when the player tries to reconnect
timeoutMessage = "Invalid password. Please wait %s before trying to connect again."
-- Wait before the player can re-join
invalidPasswordTimeout = 5 * 60 -- 5 minutes
...
AddEventHandler('Fax:ServerPassword:CheckPassword', function(Newpassword, attempts)
    ...
    if clPassword == password then
        TriggerClientEvent("Fax:ServerPassword:PassedPassword", s, true)
